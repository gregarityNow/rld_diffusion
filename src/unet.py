...
class ResnetBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, time_emb=None, class_emb=None):
        h = self.block1(x)

        if defined(self.mlp) and defined(time_emb):
            time_emb = self.mlp(time_emb)
            # h = rearrange(time_emb, "b c -> b c 1 1") + h
            # print("timeShape",time_emb.shape)
            h = time_emb.unsqueeze(-1).unsqueeze(-1) + h

        if defined(self.mlp_class) and defined(class_emb):
            class_emb = self.mlp_class(class_emb)
            # h = rearrange(time_emb, "b c -> b c 1 1") + h
            # print("classShape",class_emb.shape)
            h = class_emb.unsqueeze(-1).unsqueeze(-1) + h
...
class Unet(nn.Module):
    def __init__(
        ...
        if class_emb_dim == "oneHot":
            ...
            self.class_embedder = nn.Sequential(
                ...
                nn.SiLU(),
                nn.Linear(int(class_emb_dim/2), class_emb_dim),
            )
        ...
        for ind, (dim_in, dim_out) in enumerate(in_out):
            ...
            self.downs.append(
                nn.ModuleList(
                    [
                        resnet_block(dim_in, dim_out, time_emb_dim=time_dim, class_emb_dim=class_emb_dim),
                        resnet_block(dim_out, dim_out, time_emb_dim=time_dim, class_emb_dim=class_emb_dim),
        ...
        mid_dim = dims[-1]
        self.mid_block1 = resnet_block(mid_dim, mid_dim, time_emb_dim=time_dim, class_emb_dim=class_emb_dim)
        self.mid_attn = Residual(PreNorm(mid_dim, Attention(mid_dim)))
        self.mid_block2 = resnet_block(mid_dim, mid_dim, time_emb_dim=time_dim, class_emb_dim=class_emb_dim)
        ...
        for ind, (dim_in, dim_out) in enumerate(reversed(in_out[1:])):
            ...
            self.ups.append(
                nn.ModuleList(
                    [
                        resnet_block(dim_out * 2, dim_in, time_emb_dim=time_dim, class_emb_dim=class_emb_dim),
                        resnet_block(dim_in, dim_in, time_emb_dim=time_dim, class_emb_dim=class_emb_dim),
    ...
    def forward(self, x, t: Optional[torch.Tensor] = None, classes: Optional[torch.Tensor] = None):
        assert defined(t) or not defined(self.time_dim), "No time embeddings given"

        batch_size = x.shape[0]
        if self.class_emb_dim is None:
            classes_embedded = None
        elif classes is None:
            classes_embedded = torch.zeros((batch_size,self.class_emb_dim)).to(device)
        else:
            classes = classes.unsqueeze(-1).to(torch.float32)
            classes_embedded = self.class_embedder(classes);
            classes_embedded[torch.where(torch.rand(batch_size)<self.thresh)] = 0
        ...
        for block1, block2, attn, downsample in self.downs:
            x = block1(x, t, classes_embedded)
            x = block2(x, t, classes_embedded)
            x = attn(x)
            h.append(x)
            x = downsample(x)

        # bottleneck
        x = self.mid_block1(x, t, classes_embedded)
        x = self.mid_attn(x)
        x = self.mid_block2(x, t, classes_embedded)
